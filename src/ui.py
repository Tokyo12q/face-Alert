...
class ProctorUI:
    ...
    def convert_frame(cv_frame, width, height):
        """Converts an OpenCV BGR frame to a PhotoImage for Tkinter."""
    ...
    def draw_overlays(frame, faces, primary_face, eye_coords, status):
        ...
        for face in faces:
            ...
            box_color = color if is_primary else (255, 0, 255) # Magenta for strangers
            ...
            if not is_primary:
                ...
                # Draw 3D mesh landmarks for primary face
                # This helps visualize why the tracking is more accurate
                # Only draw a subset for clarity
                for i, lm_pixel in enumerate(face['coords']):
                    if i % 10 == 0: # Sparse mesh for visibility
                        cv2.circle(frame, (int(lm_pixel[0]), int(lm_pixel[1])), 1, (0, 255, 255), -1)
                
                # Highlight the eyes specifically
    ...
    def create_hud(main_frame, binary_eye_zoom):
        # Resize binary zoom to match height or a fixed size
        ...
        if len(binary_zoom_resized.shape) == 2:
            binary_zoom_resized = cv2.cvtColor(binary_zoom_resized, cv2.COLOR_GRAY2BGR)
            
        # Add labels
        cv2.putText(main_frame, "LIVE FEED", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        cv2.putText(binary_zoom_resized, "EYE TRACKING ZOOM", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
