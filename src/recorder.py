...
class ProctorRecorder:
    def __init__(self, db_path="proctoring_log.db", evidence_dir="evidence"):
        ...
        self._clear_evidence() # Automatically empty folder on start
    ...
    def _clear_evidence(self):
        """Deletes all files in the evidence directory on startup."""
    ...
    def log_event(self, event_type, description, integrity_score, frame=None):
        ...
        self.cursor.execute('''
            INSERT INTO events (timestamp, event_type, description, integrity_score, screenshot_path)
            VALUES (?, ?, ?, ?, ?)
        ''', (timestamp, event_type, description, integrity_score, screenshot_path))
        self.conn.commit()

    def generate_summary(self):
        self.cursor.execute('SELECT COUNT(*) FROM events WHERE event_type LIKE "%VIOLATION%"')
